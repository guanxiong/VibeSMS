...
- 当前版本：`0.10.0`，首页使用原地申请弹窗并公开实时显示剩余自动签发名额，页面每 30 秒及重新可见时同步；公开页面支持中英文切换与语言持久化，并完成标题断词、字号和移动端换行加固。同时启用人工激活码兜底、用户 Key 收件箱、Agent USB 自动配置、Android 绑定和 Agent Inbox/OTP API；Android Terminal APK 为 v0.4.9
