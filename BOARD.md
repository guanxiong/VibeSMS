...
- 2026-08-13：VibeSMS 0.12.0 已部署生产。用户可在 Key 收件箱为当前 Key 配置飞书/Lark 官方机器人 Webhook、启用状态和 1–10 个关键词；仅新短信命中任一关键词时创建持久化投递，失败指数退避并最多重试 5 次。查询只显示地址脱敏尾号，提供无真实短信的测试发送、投递状态和删除配置；同时修复语言切换链接继承 `#key` 的短暂凭据残留。32 项测试与 390px 中英文验收通过，生产新表为空且未授权接口为 401，备份为 `gateway-20260812T220009Z.db`。
- 2026-08-13：VibeSMS 0.12.1 已部署生产。Key 收件箱中的飞书 Webhook 机器人配置默认折叠，标题行保留配置状态，并提供中英文展开/收起文案与方向提示；配置读取或保存错误时自动展开，避免隐藏错误。390px 中英文实测确认关闭态不渲染表单、展开态完整可用且无横向溢出，32 项自动化测试通过；未授权 Webhook 接口仍为 401，自动签发额度保持 50，部署前备份为 `gateway-20260813T041103Z.db`。
