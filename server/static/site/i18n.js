(() => {
  ...
  const translations = {
    ...
    "无需注册账户。自动名额可用时，提交后会立即显示一次真实 Key；额度用尽后转入人工审核。": "No account required. When capacity is available, a real Key is shown once immediately after submission; otherwise the request enters manual review.",
    ...
    "无需新建账户。自动名额可用时，提交后立即生成真实 Key；额度暂停或用尽时，申请会进入人工审核队列。": "No new account is required. A real Key is issued immediately when capacity is open; otherwise your request enters the review queue.",
    "你需要提交": "You provide",
    "邮箱、自己的手机号和简要用途": "Email, your phone number, and a short use case",
    "自动名额可用": "When capacity is open",
    "立即获得只显示一次的真实 Key": "Receive a real Key that is shown once",
    "下一步": "Next",
    ...
    "权限": "Permissions",
    "只读，不含管理功能": "Read-only, no admin access",
    ...
    "生效日期：2026 年 8 月 12 日。VibeSMS 是一个仍在迭代中的 Public Beta；本页用清晰、可核验的语言说明当前服务处理什么数据，以及你在申请前应当知道什么。": "Effective August 12, 2026. VibeSMS is an evolving public beta. This notice explains in clear, verifiable terms what data the service processes and what you should know before applying.",
    "使用边界": "Acceptable use",
    ...
    "数据用于签发和管理 Key、将事件按已绑定的设备与 SIM 隔离、向对应 Key 的 Agent 或收件箱提供读取能力，以及维护终端在线状态和安全审计。当前服务把这些数据保存在部署实例的 SQLite 数据库中。": "Data is used to issue and manage Keys, isolate events by bound device and SIM, provide access to the corresponding agent or inbox, maintain terminal presence, and support security auditing. The current service stores this data in the deployment's SQLite database.",
    ...
    "一个用户 Key 仅可读取其绑定号码、设备与 SIM 卡槽的事件。Android 终端使用独立的上传凭据，不能读取收件箱。管理认证可查看运维所需的记录，因此服务管理员可能接触存储的事件内容。Key 泄露时，请立即在管理端轮换或禁用，或联系部署运营者处理。": "A user Key can read only events for its bound number, device, and SIM slot. Android terminals use separate upload credentials and cannot read the inbox. Administrative access can view records needed for operations, so service administrators may access stored event content. If a Key is exposed, rotate or disable it immediately or contact the deployment operator.",
    "浏览器与第三方": "Browser storage and third parties",
    "当前公开站点不加载广告、统计像素或第三方分析脚本。推广链接中的": "The public site does not load ads, tracking pixels, or third-party analytics. The",
    "仅在你提交测试 Key 申请时，作为后台预先创建的活动标签写入申请记录；服务不会存储完整 URL、IP、User-Agent 或浏览器指纹。Key 收件箱会将你主动输入或通过 URL 片段导入的 Key 临时放在当前浏览器标签页的": "value in a campaign link is written to the application only when you submit a test Key request, using a campaign label pre-created by the operator. The service does not store the full URL, IP address, User-Agent, or browser fingerprint. The Key Inbox temporarily stores a Key you enter or import through a URL fragment in the current tab's",
    "；关闭该标签页后会清除。该 Key 不会被放入页面 URL 查询参数发送给服务端。": ". It is cleared when the tab closes. The Key is never placed in a URL query parameter sent to the server.",
