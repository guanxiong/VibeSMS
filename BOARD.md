...
- 2026-08-13：VibeSMS 0.9.1 完成中英文排版加固并部署生产：英文相邻标题短语改用真实空格，中文手机端按完整短语换行，收紧首页与 Beta 大标题宽度，并提高语言开关、代码页签、复制操作和说明文字字号。首页、申请、激活、收件箱、隐私页在 1440px/390px 下均无页面溢出或控件裁切；30 项测试通过，备份为 `gateway-20260812T191140Z.db`。
- 2026-08-13：VibeSMS 0.9.2 调整 HOW IT WORKS 标题的语义分组，“把你的号码”保持为完整短语；390px 生产页面实测第一行“三步，把你的号码”、第二行“交给 Agent。”，无横向溢出。30 项测试通过，备份为 `gateway-20260812T192536Z.db`。
- 2026-08-13：VibeSMS 0.10.0 将首页测试 Key 申请固定为原地弹窗，并通过公开状态接口安全返回剩余自动签发数量；首页加载、每 30 秒、页面重新可见、弹窗打开及成功签发后都会刷新。生产实测显示剩余 4 个，弹窗与首页数量一致；额度为 0 时切换为人工审核提示。30 项测试通过，备份为 `gateway-20260812T194229Z.db`。
- 2026-08-13：VibeSMS 0.11.0 已实现每个匿名领取设备最多自动签发一次。浏览器仅持久化随机领取标识，服务端保存加域 SHA-256 摘要，并由事务检查与 SQLite 唯一索引共同阻止并发重复领取；重复设备和无法持久化标识的申请进入人工审核且不扣额度。首页、独立申请页、后台审核原因和中英文隐私说明同步更新，31 项测试通过；生产剩余自动额度保持 4 个，备份为 `gateway-20260812T213919Z.db`。
