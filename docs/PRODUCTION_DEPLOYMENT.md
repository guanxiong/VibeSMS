...
- 当前版本：`0.12.1`，Key 收件箱的飞书 Webhook 设置区默认折叠，标题行持续显示转发状态，可展开管理地址、1–10 个关键词、启用状态与投递记录；保存或读取错误时自动展开。只有命中任一关键词的新短信进入持久化投递队列，失败采用指数退避并最多重试 5 次。首页继续使用原地申请弹窗，每个匿名领取设备限自动签发一次；Android Terminal APK 为 v0.4.9
